...
- **Full JCR table is gitignored** (`data/jcr-if.js`, `data/jcr-if.json`, `data/JCR*.pdf` — Clarivate-licensed, local-only, never commit). Publishing all ~13.5k values would redistribute the licensed JCR database.
- **Public subset IS committed**: `data/jcr-if-public.js` (~420 ISSN keys) covers the whole RADIOLOGY, NUCLEAR MEDICINE & MEDICAL IMAGING category plus ~26 curated major medical journals. It assigns `RKG.jcrData` only when the full local table didn't load (`RKG.jcrData = RKG.jcrData || {...}`), so index.html loads `jcr-if.js` (404s on public builds) before `jcr-if-public.js`. Journals outside the subset fall back to OpenAlex with the "OA" badge.
- Yearly regeneration: `python extract_jcr.py <JCR pdf>` (full local table) then `python build_public_if.py` (public subset; commit this one). If top radiology journals suddenly show ~5 locally, the full table is missing — re-run extract_jcr.py.
